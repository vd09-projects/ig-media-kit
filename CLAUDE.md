...
Four MCP tools over one shared fetch engine. **Fetcher** (one paced ~40-item window: `web_profile_info` → `user_id` → paginate `/api/v1/feed/user/{id}/`) is the primitive; `list_reels` calls it synchronously, the **Batch runner** calls it in a background job. **Store** (CSV manifest + YAML state, per handle) is the source of truth; **Downloader** GETs the mp4 URL that already arrives in the feed JSON. Data flows: feed JSON → normalized reel → store → (on demand) download. Call-driven fill accumulates each handle to `scan_depth=90` across calls — no background worker except the async batch tool.
