...
**Status:** v2 · 2026-07-14 · research-session (huginn inline)
**Verification:** 12 load-bearing claims — 10 verified by live probe, 2 open (download-URL TTL, single-reel refresh)
...
**Rate-limit handling (no worker, no sleep) — POLITENESS IS LOAD-BEARING:** pilots proved the anonymous quota is IP-keyed and *punishes abuse* — cooldown escalates 6.6→13 min and budget degrades 48→36→12 under repeated tripping. So the tool must be deliberately gentle: within a call, pace pages (~1-2s apart), cap ~4 pages (~40 items) per call, **stop and return partial on the first 401** (never retry-hammer, never sleep inside a call). The ~6.6-min cooldown is absorbed by the spacing between manual calls; a call arriving <~7 min after the previous one returns what's stored with a "budget cooling" note. On repeated 401s, back off (don't poll the endpoint during cooldown — that extends it). Steady-state top-refresh (<40 new items) never trips it. **Store is never destructively capped** — `scan_depth=90` is the fetch-effort target; everything fetched is kept, and top-`count` is computed over the pool.
